    ...
    <div class="relative flex-shrink-0" :class="deleting ? 'opacity-25' : ''">
        @if ($type === 'bullet')
            <div class="border border-transparent">
                <button
                    class="h-10 w-10 md:h-8 md:w-8 flex items-center justify-center rounded-full border border-transparent hover:border-gray-200 hover:shadow focus:outline-none focus:border-gray-200 focus:shadow-inner"
                    :class="[
                ...
                >
                    @if ($bullet->state === 'incomplete')
                        <x-icon.incomplete x-show="state === ''" class="h-6 w-6 md:h-5 md:h-5 text-gray-400" />
                    @endif
                    <x-icon.incomplete x-cloak x-show="state === 'incomplete'" class="h-6 w-6 md:h-5 md:h-5" />

                    @if ($bullet->state === 'complete')
                        <x-icon.complete x-show="state === ''" class="h-6 w-6 md:h-5 md:h-5 text-gray-400" />
                    @endif
                    ...
                    @if ($bullet->state === 'event')
                        <x-icon.event x-show="state === ''" class="h-6 w-6 md:h-5 md:h-5 text-gray-400" />
                    @endif
            ...
            >
                ...
                <button class="h-10 w-10 md:h-8 md:w-8 flex items-center justify-center hover:bg-gray-100 focus:outline-none" @click="deleting = true" wire:click="delete">
